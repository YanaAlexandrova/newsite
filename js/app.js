jQuery(function($){
    ...
    $('#contact-me').on('click', function(e){
        e.preventDefault();
        if($(this).next('.contacts-block').css('display') == 'none') {
            $(this).next('.contacts-block').slideDown(300);
            $('.contacts-header .contact-me').addClass('active');
        }
        else {
            $(this).next('.contacts-block').slideUp(300);
            $('.contacts-header .contact-me').removeClass('active');
        }
    })

    $('#menu-top-icon').on("click", function(e){
        function openMenu(){
            $('.line').css('fill','#cecece');
            $('#l1').css('transform', 'rotate(45deg)');
            $('#l2').css('opacity','0');
            $('#l3').css('transform', 'rotate(315deg)');
         }
         function closeMenu(){
            $('.line').css('fill','#fff');
            $('#l1').css('transform', 'translateY(-16px)');
            $('#l2').css('opacity','1');
            $('#l3').css('transform', 'translateY(16px)');
         }

        if($(this).hasClass('menu-opened')){
            $(this).find('#l1').css('transform', 'rotate(0)');
            $(this).find('#l3').css('transform', 'rotate(0)');
            setTimeout(closeMenu, 300 );
            $(this).removeClass('menu-opened');
            $('.law-head .center').slideUp(300);

        }
        else {
            $(this).find('#l1').css('transform', 'translateY(0)');
            $(this).find('#l3').css('transform', 'translateY(0)');
            setTimeout(openMenu, 300 );
            $(this).addClass('menu-opened');
            $('.law-head .center').slideDown(300);
        }
    ...
    $.fn.animate_Text=function(){
        ...
        return this.each(function(){
            ...
            $this.find('span.new').each(function(i,el){
                setTimeout(function(){$(el).addClass('div_opacity');
            },80*i);
            });
        });
    };

    setTimeout(function(){$('#quest_slide1').show().animate_Text()}, 2000)

    $('.answers .faq-item h4').on('click', function(){
        if($(this).parent().hasClass('active')) {
            $(this).next().slideUp(200).parent().removeClass('active');
        }
        else {
            $(this).parent().siblings().removeClass('active').find('div').slideUp(200);
            $(this).next().slideDown(200).parent().addClass('active');
        }
    })

    $('.fancy-open').on('click',function(e){
        e.preventDefault();

        let fancyName = $(this).attr('href');

        $.fancybox.open($(fancyName));
    })

    $(window).scroll(function(){
        var wScroll=$(this).scrollTop();
        $('#fig1').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.3+'%)'});
        $('#fig2').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.2+'%)'});
        $('#fig3').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.3+'%)'});
        $('#fig4').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.3+'%)'});
        $('#fig5').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/3+'%)'});
        $('#fig6').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.3+'%)'});
        $('#fig7').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.2+'%)'});
        $('#fig8').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.3+'%)'});
        $('#fig9').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.3+'%)'});
        $('#fig10').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/3+'%)'});
        $('#fig11').css({'transform':'translate(0, -'+(wScroll-$('.law-kwiz-cont').offset().top)/1.2+'%)'});
